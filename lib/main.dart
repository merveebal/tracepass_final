...
// 🚀 Animasyon desteği için TickerProviderStateMixin ekledik
class _HomePageState extends State<HomePage> with TickerProviderStateMixin {
  ...
  // 🐉 Tarsy Animasyon Değişkenleri
  late AnimationController _tarsyController;
  late Animation<double> _tarsyAnimation;
  ...
  void initState() {
    ...
    // 🚀 Tarsy Zıplama Animasyonu Kurulumu
    _tarsyController = AnimationController(
      duration: const Duration(milliseconds: 300),
      vsync: this,
    );

    _tarsyAnimation = Tween<double>(begin: 0, end: -20).animate(
      CurvedAnimation(parent: _tarsyController, curve: Curves.easeInOut),
    );
  }

  @override
  void dispose() {
    _tarsyController.dispose(); // Belleği temizle
    super.dispose();
  }

  // 🐉 Tarsy'yi zıplatma ve selam verme fonksiyonu
  void _jumpTarsy() {
    _tarsyController.forward().then((_) => _tarsyController.reverse());
    
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: const Text("Tarsy: Merhaba Merve! Bugün dünyayı kurtarmaya hazır mısın? 🌍"),
  ...
  String _getTarsyImage() {
    ...
    if (HomePage.scanCount == 0) return '${path}tarsy_egg.png';
    if (HomePage.scanCount <= 5) return '${path}tarsy_hatch.png';
    return '${path}tarsy_happy.png';
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Stack( // 🚀 Katmanlı yapı için Stack
          children: [
            SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 120), // Alt boşluk
                ],
              ),
            ),

            // 🐉 TARSY SAĞ ALT KÖŞE (Animasyonlu)
            Positioned(
              bottom: 110,
              right: 20,
              child: GestureDetector(
                onTap: _jumpTarsy, // 🚀 Tıklayınca zıplat ve selam ver
                child: AnimatedBuilder(
                  animation: _tarsyAnimation,
                  builder: (context, child) {
                    return Transform.translate(
                      offset: Offset(0, _tarsyAnimation.value), // 🚀 Zıplama hareketi
                      child: Column(
                        children: [
                          Image.asset(
                            _getTarsyImage(),
                            height: 100, // Köşeye yakışan boyut
                            fit: BoxFit.contain,
                            errorBuilder: (context, error, stackTrace) => const Icon(Icons.pets, size: 50, color: Colors.grey),
                          ),
                          Text(
                            "Tarsy",
                            style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: forestGreen.withOpacity(0.6)),
                          ),
                        ],
                      ),
                    );
